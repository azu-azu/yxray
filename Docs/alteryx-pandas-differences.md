...
| 観点 | 規則 | 状態 |
|------|------|------|
| 重複する検索値 | 同じ検索値が複数の lookup 行にあるときは**後の行**が有効（辞書的上書き）。RMF 設定に依らない | **golden 突合で検証済み**（apple×2 → X,Y を両 RMF 設定で実行、どちらも Y） |
| 採用マッチ（FindAny） | **走査で最初に完了するマッチ**（= マッチの終了位置が最も左）の検索値の行 — lookup 順ではなく、**RMF 設定にも依らない** | **golden 突合で検証済み**（5行データ・入れ子 app/apple とも、RMF=True / False の両設定で同一出力を実測）。ただし「開始位置が最も左」モデルとの判別ケースは未実施（下記の保留事項参照） |
| `ReplaceMultipleFound` | **FindAny + Append では True/False で出力差が観測されていない**（3種の golden × 両設定すべて同一）。Replace モード専用の設定である可能性が高い | FindWhole の RMF=False のみ未実測（下記の保留事項参照） |
...
- **入れ子の検索値**（lookup: app→SHORT, apple→LONG、target "apple pie"）:
  **SHORT** — 開始位置は同じ(0)でも、先に完了するマッチ（終了位置が左）が
  勝つ（`test_find_any_nested_needles_earliest_end_wins` で固定）。この実測に
  より採用規則は「開始位置」より「終了位置」で解釈するのが自然になった
  （両モデルの判別ケースは未実施 — 下記の保留事項参照）
...
**検証状態**: `RMF=True → keep="last"`（lookup 表の並び順で最後の重複が勝つ）
...
（重複する検索値は後の行が有効・RMF は FindAny + Append では出力に影響
しない）からの予測は、**「FindWhole でも RMF=False で last が勝つ」に変わった**
（当初の keep="first" 予測から更新）。実測で確定したら、first が正なら
現状のまま消し込み、last が正なら `scaffold.py` の
`keep = "last" if replace_multiple_found else "first"` を常に `"last"` へ直す。
...
- **1 target = 1 出力行** — FindReplace は join ではないので、複数の
  lookup 行にマッチしても行は増えない。採用されるのは走査で最初に完了する
  マッチ（終了位置が最も左）の検索値の行（RMF に依らず・golden 検証済み）。
...
- [ ] **FindWhole の RMF=False** — 統一モデル（重複する検索値は後の行が有効、
  RMF は FindAny + Append では出力に影響しない）からの予測は「RMF=False でも
  last が勝つ」に変わった（当初の keep="first" 予測から更新）。同じキーを
  複数行（値違い）入れた FindWhole ワークフローを RMF=False で実行し、
  最初の行（keep="first" が正 — 現状のまま消し込み）か最後の行（RMF 無関係に
  last — `scaffold.py` の keep を常に `"last"` へ変更）かを実測する。
  確定したら `_findreplace_whole_append` の NOTE と
  `test_scaffold_findreplace_whole_match_first_match_dedup` も更新する
- [ ] **開始位置 vs 終了位置の判別（FindAny）** — 実測済みの入れ子ケース
  （app/apple、同じ開始位置）は「終了位置が左」で説明したが、「同じ開始なら
  短い方」という開始位置ベースの解釈とまだ区別できていない。判別には
  「後から始まって先に終わる」検索値が要る: target "apple pie"、lookup に
  `apple`（位置0-5）と `ppl`（位置1-4）— **終了位置モデルなら ppl、開始位置
  モデルなら apple** が採用される。現実装は終了位置モデル
  （`test_find_any_earliest_end_beats_earliest_start_assumption` で固定、推定）。
  あわせて入れ子の並び順を逆にしたケース（lookup: apple, app の順）も取ると、
  「同点は先の行」タイブレークの検証になる
  （`test_find_any_nested_needles_reversed_order_assumption` で固定、推定）
