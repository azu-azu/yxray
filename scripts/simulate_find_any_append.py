...
- needle = lookup 側の search_field 値が targets 側の find_field に
  部分文字列として含まれるか
- 採用規則: 走査で最初に完了するマッチ（終了位置が最も左）の検索値の行が
  勝つ — lookup 順ではない（入れ子の app/apple では短い app が勝つ）
- 同じ検索値が複数の lookup 行にあるときは後の行が有効（辞書的上書き）
- ReplaceMultipleFound は FindAny + Append では出力に影響しない
  （3種の golden × True/False 両設定で同一出力）
- NoCase=True は大小無視でマッチ（採用規則は維持）
- 空文字・NULL の検索値は無視される
- 出力列は「元の Targets 列 + append_fields」のみ（検索値の列は含まない）

推定のまま残っているのは、終了位置が同点のタイ（先の lookup 行を維持）と、
「開始位置 vs 終了位置」の判別（後から始まって先に終わる検索値のケース）のみ。
"""
...
def simulate_find_any_append(
    ...
    *,
    ...
    複数の lookup 行にマッチしたとき採用されるのは「走査で最初に完了する
    マッチ」= マッチの終了位置が target 文字列中で最も左の検索値の行
    （golden 突合で検証済み — lookup 順ではない点に注意。開始位置が同じ
    入れ子の検索値 app/apple では、先に終わる短い app が勝つ）。
    同じ検索値が複数の lookup 行にあるときは後の行が有効
    （辞書的上書き — golden 突合で検証済み）。
    replace_multiple_found（Alteryx の ReplaceMultipleFound）は FindAny +
    Append では True/False どちらでも出力が変わらないことが golden で実測
    されているため、判定には使わない。終了位置が同点のタイは先の lookup 行を
    維持する（推定 — golden 未取得）。
    """
    ...
    # 採用中マッチの target 文字列内での終了位置（-1 = 未マッチ）
    best_end = pd.Series(-1, index=targets.index, dtype="int64")
    ...
    for lookup_id, values in zip(
        ...
        # 採用されるのは「走査で最初に完了するマッチ」= マッチの終了位置が
        # target 文字列中で最も左の検索値の行（golden 実測: 開始位置が同じ
        # 入れ子の app/apple では短い app が勝つ。5行 golden の最左採用も
        # この規則で説明できる）。ReplaceMultipleFound は FindAny + Append
        # では出力に影響しないことが実測されているため判定に使わない。
        # 終了位置が同点のタイは先の lookup 行を維持する（推定・golden 未取得）。
        end = pos + len(needle_cmp)
        fill = contains & ((best_end < 0) | (end < best_end))
        if fill.any():
            winning_lookup_id[fill] = lookup_id
            matched_needle[fill] = needle
            best_end[fill] = end[fill]
