...
に部分文字列として含まれる）、採用規則（RMF 設定に依らず target 文字列中で
最も左に現れた検索値の行が勝つ — lookup 順ではない）、出力列が
「元の Targets 列 + append_fields」のみで検索値の列を含まないことは、
いずれも実 Alteryx の golden 出力との突合で検証済み。
ReplaceMultipleFound が効くのは複数の lookup 行が同じ最左位置にマッチする
とき（同じ検索値の重複行など）で、True=後の行 / False=先の行を採用
（FindWhole の重複キー実測と整合する統一モデル。この同位置タイ自体の
golden は未取得のため推定）。
"""
...
def simulate_find_any_append(
    ...
    *,
    ...
    replace_multiple_found: bool = True,  # Alteryx の ReplaceMultipleFound。採用は常に text 内最左のマッチ（RMF に依らず・golden 検証済み）。同位置タイのみ True=後の lookup 行 / False=先の行（推定）
    ...
    複数の lookup 行にマッチしたとき採用されるのは、replace_multiple_found の
    設定に依らず「target 文字列中で最も左（先頭に近い位置）に現れた検索値」の行
    （golden 突合で RMF=True / False とも同一出力を実測 — lookup 順ではない
    点に注意）。replace_multiple_found（Alteryx の ReplaceMultipleFound）が
    効くのは、複数の lookup 行が同じ最左位置にマッチするとき（同じ検索値の
    重複行や、同位置で始まる検索値）: True なら後の行で上書き、False なら
    先の行を維持（FindWhole の重複キー実測と整合する統一モデル。この同位置
    タイ自体の golden は未取得のため推定）。
    """
    ...
    for lookup_id, values in enumerate(lookup.itertuples(index=False, name=None)):
        ...
        # 採用されるのは「target 文字列中で最も左（先頭に近い位置）に現れた
        # 検索値」の行で、これは ReplaceMultipleFound に依らない
        # （同一の golden データを RMF=True / False の両設定で実測、出力は同一）。
        # RMF が効くのは複数の lookup 行が同じ最左位置にマッチするとき
