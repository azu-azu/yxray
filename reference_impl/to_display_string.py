...
Double 列に文字列プレースホルダを混ぜた列（`IF IsEmpty([Size]) THEN "-"
ELSE [Size] ENDIF` の結果など）をそのまま出力すると Alteryx と食い違う。
...
2つは合成して使う。順序は「文字列化 → 補充」:
    ...
    df["Size"] = fill_empty(to_display_string(df["Size"]), "-")
...
#   "SM"      数値でないテキストは残る（to_numeric なら NaN に化ける）
#   "1.5"     小数部がゼロでないので残る
...
def to_display_string(series: pd.Series) -> pd.Series:
    ...
    **すでに文字列になっている値も拾う。** CSV を `dtype=str` で読んだ、
    ...
    のが要点。coerce は `"SM"` のような数値でないテキストを黙って NaN にし、
