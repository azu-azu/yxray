...
function downloadSummaryExcel() {
  ...
  var bytes = _makeXlsx(xlSheets);
  var blob = new Blob([bytes], {type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});
  var url = URL.createObjectURL(blob);
  var a = document.createElement('a');
  a.href = url; a.download = 'summary_' + baseName + '_' + ts + '.xlsx';
...
class HTMLRenderer:
    ...
    def _node_to_dict(self, node: AlteryxNode) -> dict[str, Any]:
        return {
            ...
            "short_type": classify_tool(node.tool_type)[0],
    ...
    def _node_diff_to_dict(self, nd: NodeDiff) -> dict[str, Any]:
        return {
            ...
            "short_type": classify_tool(nd.old_node.tool_type)[0],
            "field_diffs": [
