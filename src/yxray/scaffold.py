...
def _read_stmt(target: str, path: str | None, path_expr: str) -> str:
    ...
    if ext == ".shp":
        # GDAL refuses a .shp whose .shx sidecar is missing; Alteryx reads it
        # anyway, so the scaffold must opt in to restoring the index.
        return (
            'os.environ.setdefault("SHAPE_RESTORE_SHX", "YES")'
            "  # read .shp without .shx\n" + stmt
        )
    return stmt
...
def _collect_metadata(
    ...
    if not has_spatial:
        has_spatial = any(
            pathlib.Path(p).suffix.lower() in _SPATIAL_EXTS
...
def scaffold_simple_blocks(
    ...
    stdlib: list[str] = []
    if has_browse:
        stdlib.append("import logging")
    if any("SHAPE_RESTORE_SHX" in line for block in blocks for line in block.lines):
