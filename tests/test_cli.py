...
def test_explain_md_scaffold_is_indented_for_def_body(
    ...
    """The .md python code is bare (no ```python fence) and indented so it
    can be pasted directly under a def statement."""
    workflow = tmp_path / "wf.yxmd"
    workflow.write_bytes(FILTER_EXPR_YXMD)
    out_dir = tmp_path / "out"
    ...
    assert '    """Scaffold generated by yxray from wf.yxmd"""' in md
    assert "\n    df2 = df1[df1[\"Country\"] == 'Japan']" in md
    # Python lines are all indented for a def body; only XML fences and
    # headings sit at column 0
...
def test_explain_md_interleaves_node_xml_after_each_tool(
    ...
    assert result.exit_code == 0
    md = (out_dir / "wf.md").read_text(encoding="utf-8")
    # XML source blocks are present for both tools
    assert md.count("```xml") == 2
    assert '<Node ToolID="1">' in md
    assert '<Node ToolID="2">' in md
