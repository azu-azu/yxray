...
def test_explain_md_scaffold_is_indented_for_def_body(
    ...
    """The .md scaffold can be pasted directly under a def statement."""
    workflow = tmp_path / "wf.yxmd"
    workflow.write_bytes(FILTER_EXPR_YXMD)
    out_dir = tmp_path / "out"
    ...
    assert result.exit_code == 0
    md = (out_dir / "wf.md").read_text(encoding="utf-8")
    code_block = md.split("```python\n", maxsplit=1)[1].split("\n```", maxsplit=1)[0]
    assert code_block.startswith('    """Scaffold generated by yxray from wf.yxmd"""')
    assert "\n    df2 = df1[df1[\"Country\"] == 'Japan']" in code_block
