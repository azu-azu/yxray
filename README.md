...
- **Preamble**: imports and `ENV = os.getenv("APP_ENV", "test")`. The `SelectColumnEdit` / `apply_select_edits` helper definitions are **not** embedded — Select blocks carry a NOTE comment pointing to the reference implementation in `reference_impl/select_edits.py`, which you copy into your project
