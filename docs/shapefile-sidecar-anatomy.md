...
> **2026-07-31 追記: MapInfo TAB も同じ扱いにした。**
> `.tab` を `SPATIAL_EXTS` に追加した際、サイドカーガードを
> `SIDECAR_EXTS`(`_io.py`)へ一般化し、`.tab` には `.dat` / `.DAT` を
> 要求するようにした。MapInfo TAB も1ファイルでは完結せず、
> `.tab` はヘッダ、図形は `.map`、属性表は `.dat` に入っている。
> 生成コードの形は .shp と同じで、変数名(`_shp` / `_tab`)と
> サイドカー拡張子だけが変わる。
