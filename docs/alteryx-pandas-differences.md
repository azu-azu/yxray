...
上の表にある `" 1.0"`（前後の空白）や `"1,000.0"`（桁区切り）は意図的に対象外
なので、そういう列は呼び出し側で整えてから渡す。どうしても数値パースが必要な
場合だけ `pd.to_numeric()` を使い、`apply_select_edits` が Alteryx の
Conversion Error に対して使っているのと同じく、**変換で null が増えていないかを
併せて見る**:
