...
### 生成コード側の関数化はいつ許されるか — 全ツール共通の基準

`fill_empty` を作って `is_empty` を作らなかった上の判断は Formula 限定の話では
なく、生成器全体の方針である。「毎回同じパターンが出るから `select_edits` のように
関数にまとめるか、それとも毎回ベタ書きか」を検討するときは、まず**選択肢が4つ
ある**ことを確認する。`reference_impl` 形式はその中でいちばん重く、既定ではない。

| 段 | 実体 | 生成コードへの影響 | 利用者のコスト | 実例 |
|---|---|---|---|---|
| (a) 生成器側の関数 | `scaffold/*.py` 内の private 関数・定数 | **なし**（出力は同一） | ゼロ | `_geoseries_expr()` / `_spatial_field_note()` / `_SPATIAL_INFO_ITEMS` |
| (b) `GeneratedCode.helpers` | `def` を生成 `.py` に埋め込む（`_emit_helper_defs` が `main()` の上へ持ち上げ、`.md` は呼び出し直前にインライン） | 呼び出し1行＋定義が同ファイル内 | ゼロ（自己完結） | TextInput の `build_text_input_df_<ToolID>()` |
| (c) `reference_impl/` | 定義は**生成しない**。NOTE コメントで場所を指し、利用者がファイルをコピーする | 呼び出しのみ | **コピー手順が1個増える** | `apply_select_edits` / `find_any_append` / `fill_empty` |
| (d) ベタ書き | 素の pandas + コメント | 全部見える | ゼロ | Filter / Create Points / Spatial Info |

#### 既定は (a) — DRY は生成器側で取り、出力側では取らない

通常のアプリコードでは重複はバグの温床で、抽象化はほぼ無料（同じリポジトリ内で
定義に飛べる）。生成コードは損得が逆になる。**読み手は「Alteryx の挙動と一致して
いるか」を1ブロックずつ golden と突合して検証する**ので、ブロックがローカルに完結
しているほど検証が速く、抽象化は検証の単位を壊す。したがって:

- 繰り返しは (a) で潰す。出力が1文字も変わらないのでコストがゼロ。
- 出力側（(b)/(c)）へ出すのは「ベタ書きすると静かに間違う」ときだけ。
- 判断軸は**出現回数ではなく壊れやすさ**。rule of three はここでは使わない
  （3箇所に出ても、どれも組み込み1行なら関数化しない）。

#### 昇格の条件 — すべて満たすときだけ

1. pandas / geopandas の組み込み一発では書けない
2. ベタ書きだと静かに間違う（dtype・CRS・適用順序・分岐がある）
3. **設定ではなくメカニズムである** — ツールの config は呼び出し側にデータとして
   残ること
4. golden か `tests/test_reference_scripts.py` で挙動を固定できる
5. 複数ノード／複数ツールから繰り返し出る

3 が特に落としやすい。`apply_select_edits` が成立しているのは、Select の設定が
`_COLS_3 = [SelectColumnEdit(...)]` というデータリテラルとして生成コードに残り、
drop / 型変換 / rename の**機構だけ**が向こう側へ行っているからである。FindReplace が
`case_sensitive=` や `collect_match_diagnostics=False` を省略せず明示出力するのも
同じ理由（15章）— 設定はレビューできる場所に置く。隠してよいのは機構だけ。

1つでも欠けたら (d) ベタ書き + コメント。重複が気になるなら (a) で潰す。

#### 満たした場合の置き場所

- 定義を生成コードに埋めて困らない（自己完結し、挙動を外で育てる必要がない）
  → **(b) `GeneratedCode.helpers`**。コピー手順が要らないぶん (c) より軽い。
- 長い／利用者のプロジェクト側で育てる／golden で挙動を固定したい
  → **(c) `reference_impl/`**

`reference_impl` は定義を生成しないぶん、**1個増えるごとに利用者の手順が1個増える**。
上で `is_empty` を却下した2つ目の理由（Filter のほぼ全ブロックがコピーを要求する
ことになる）は、まさにこのコストである。

#### 判定例

| 対象 | 1 組込無 | 2 静かに壊れる | 3 機構 | 4 golden | 5 反復 | 結論 |
|---|---|---|---|---|---|---|
| `apply_select_edits` | ✓ | ✓（型変換・適用順） | ✓ | ✓ | ✓ | (c) |
| `find_any_append` | ✓ | ✓（行の増減・NaN 誤マッチ） | ✓ | ✓ | ✓ | (c) |
| `fill_empty` | ✓ | ✓（dtype） | ✓ | ✓ | ✓ | (c) |
| `is_empty`（却下） | ✗ | ✗（マスクは常に bool） | — | — | ✓ | (d) |
| TextInput の行データ | — | — | 設定だが**隠さない**（定義は同ファイル内、`.md` はブロック内に並べる） | — | ✓ | (b) |
| Spatial Info の `.centroid` | ✗ | ✗ | — | ✗（SpatialObj は CSV 比較対象外） | △ | (d) — 18章 |
| 空間フィールド解決（`_geoseries_expr`） | ✓ | ✓（CRS） | ✓ | △ | ✓（3箇所） | 当面 (a)。昇格条件は `docs/scaffold-architecture.md` |
...
それでも文字列化を先に置くのは、`fill_empty()` が dtype を保つヘルパーで、
`Int64` の列に `"-"` を入れると **TypeError で落ちる**ため。先に文字列に
しておけばプレースホルダは常に入る。

### 自動適用はしない — 適用対象は「セルの中身」でガードする

scaffold はこの呼び出しを**生成しない**。どの列が「表示用の文字列」になるかは
Alteryx XML だけでは決まらず実データの型に依存するため、判断はレビューする
人間の責任範囲（16章と同じ立て付け）。

そのうえで、`to_display_string()` 自身は**列の dtype ではなく各セルの中身が
実数オブジェクトかどうか**で対象を決める。「ID・コード・日付列には使わない」
という運用ルールに任せず、型で落とす。

> **列 dtype でガードしてはいけない。** 生成コードが作る列は object に
> なりがちで（`np.where` の出力、プレースホルダを混ぜた列など）、中身が本物の
> float でも `is_numeric_dtype()` は False を返す。列 dtype で足切りすると
> `pd.Series([1.0, 2.0, None], dtype="object")` が丸ごと素通りし、**この
> ヘルパーが必要な場面でちょうど何もしなくなる**（2026-07-29 修正）。

セル単位で見ることで、次の事故は型の時点で起こらない。いずれも例外ではなく
**静かに間違った値**になる種類のもの:

| 入力 | ガードが無い場合 | 現在の挙動 |
|---|---|---|
| `"001"`（ゼロ埋めコード） | `"1"` | `"001"`（先頭ゼロは触らない） |
| `True` / `False` | `"1"` / `"0"` | `"True"` / `"False"` |
| `2024-01-01` | `"1704067200000000"` | `"2024-01-01"` |
| `NaT` | `"-9223372036854775808"` | `<NA>` |
| `1e300` | `TypeError` で落ちる | `"1e+300"`（Int64 範囲外は変換しない） |

実装上の注意が3点ある。いずれもテストで固定済み:

- **`bool` は `int` のサブクラス**なので明示的に弾く。`isinstance(True, Real)`
  は True を返す。
- **元の dtype のまま `.where()` してはいけない。** 日付列で非該当セルが `NaT`
  になり、`pd.to_numeric` がそれを int64 の最小値として読む。さらに `.abs()` が
  桁あふれして負のまま返るので、`Int64` 範囲ガードもすり抜ける。object に
  変換してから `.where()` すれば非該当は `NaN` になる。
- **代入は `.to_numpy()` で位置指定する。** Series のまま渡すと index で整列
  するため、フィルタや `concat` を経て重複 index を持つ列で `ValueError` になる。
...
値としては数値なのにセルが `"1.0"` という**文字列**になっている列は珍しくない
（CSV を `dtype=str` で読んだ、前段の `np.where` で文字列化された、など）。
`to_display_string()` はこれも処理する。ただし数値としてパースするのではなく、
**「正準な10進表記で小数部がゼロ」の表記だけ小数部を落とす**:
...
表示用の変換なので許容しているが、そういう列には通さないこと。
...
- **前置するには「この列は数値しか入っていない」と知っている必要がある。**
  間違っていたら静かにデータが消える。
- **前置しなければ何も知らなくてよい。** パターンに合わない表記は素通りする。

したがって**既定は前置なし**。前置するのは「`" 1.0"` や `"1.50"` のような
非正準な表記まで揃えたい」かつ「数値列だと確認済み」のときだけで、そのときは
次項の null 増加チェックとセットにすること。
...
NaN だけになった場合も `float64` になるため、見るべきは中身の件数:

```python
print(numeric.notna().sum())   # 0 なら「数値オブジェクトとしては1件も拾えていない」
```

`" 1.0"`（前後の空白）や `"1,000.0"`（桁区切り）は意図的に対象外なので、
そういう列は呼び出し側で整えてから渡す。前項の判断で `pd.to_numeric()` を
前置すると決めた場合は、`apply_select_edits` が Alteryx の Conversion Error に
