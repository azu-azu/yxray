...
この不変条件を **入口で確立する** 責務と、
**信頼して使う** 責務は、モジュールごとに分かれている。

| モジュール | 責務 |
| --- | --- |
| `_io.py` | ファイル由来の空間データを EPSG:4326 にする(確立) |
| `_spatial.py` / Create Points | 作った点に EPSG:4326 を付ける(確立) |
| `_spatial.py` / Spatial Match | すでに揃っている前提で空間結合だけする(信頼) |

`_spatial.py` の先頭 docstring にも、この分担が明記されている。

この設計にしない場合、Spatial Match のたびに毎回こうなる。
...
不変条件を信頼した **意図的な責務分担** である
([Design invariant](#design-invariant) 参照)。
...
現状の scaffold が生成する Spatial Match は述語ベースだけなので、
**生成コードの範囲ではこの問題は起きない**。
