...
**実測で裏取り済み(2026-08-05)**。実ワークフローの1行で、この
`.centroid`(度空間の平面重心)の出力を Alteryx の `Centroid` 列と
突き合わせたところ、小数点以下14桁まで一致した。つまり
「差が無視できる」ではなく、**Alteryx 自身が平面重心を計算している**
ことが確認できたということである。

**ただしこれは重心がポリゴンの内側にある場合の話**(2026-09-01 追記)。
面積重心が凹ポリゴンで自分自身の**外**に出るケースでは、Alteryx は
その重心をそのまま使わない。実測(複数行)から、Alteryx は「重心が外に
出ていたら、最近傍の境界点の方向へ内側へ押し戻す。押し戻す距離は
`min(0.01 マイル ≒ 16.09344 m, 押し戻し方向にポリゴンを貫通する区間の
半分)`」という補正を掛けていると推定できる状態まで裏取りできている
(独立した2回の実測で、対象13行中12行が誤差数cm以内、詳細は
...
**既知のギャップ(2298行中13行)— 原因は特定済み、実装は未着手
(2026-08-22 発見 → 2026-09-01 原因特定)**: `.centroid` は凹ポリゴンだと
自分自身の外に出ることがある。この13行は `_dst_m.contains(_src_m)` が
正しく `False` を返す一方、golden 側は**この13行も含め全2298行が負**
だった(符号だけでなく絶対値も golden と大きくズレる)。

原因は Distance ではなく [Spatial Info の `Centroid` 生成](#spatial-info-は不変条件を宣言し直す)
側にあると判明した: Alteryx は面積重心がポリゴン外に出ると内側へ
押し戻す補正を掛けており、`gen_spatialinfo()` はこの補正を実装していない
ため、Distance に渡る `Centroid` の値そのものが Alteryx と食い違う。
Distance 自身の符号ロジック(このすぐ上)は正しい — 間違った入力を
正しく計算しているだけ、という切り分けになる。

補正アルゴリズムは独立した2回の実測(対象13行中12行が誤差数cm以内で
一致)まで裏取りできているが、実装するかは未決定。`Centroid` は
SpatialObj 型で golden CSV に出ないため、実装してもこの13行の
