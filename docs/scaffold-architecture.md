...
#### MetaInfo による答え合わせ(2026-08-10)

上記の穴を塞ぐのではなく、**踏んだときに気づける**ようにした。

Alteryx は各ノードの `<Properties><MetaInfo connection="Output">` に、その
ノードの出力スキーマをキャッシュとして書き出す。パーサがこれを読み
(`AlteryxNode.meta_fields`)、`gen_spatialinfo()` は生成した列名をそこに
記録された名前と突き合わせる。

| MetaInfo | 生成コードのコメント |
| --- | --- |
| 無い(未解決のワークフロー) | 従来どおり。リネームしたなら「ベストエフォート」の NOTE |
| 有る・一致 | 「**confirmed by this node's own output MetaInfo**」— 推測ではなく事実として提示 |
| 有る・不一致 | **WARNING**。MetaInfo 側の名前を示し、手でリネームするよう促す |

**列名を決めるのは常に祖先の走査であって、MetaInfo ではない**。MetaInfo は
Designer がメタデータを解決したときに書かれる**キャッシュ**で、一度も開かれて
いないファイルには存在せず、Configuration に対して古くなり得る。生成の権威に
すると「MetaInfo があるとき」と「無いとき」で列名の決まり方が2系統になり、
食い違ったときの調停ロジックまで必要になる。検証専用に留めれば経路は1本のまま、
MetaInfo が無ければ黙ってスキップするだけで済む。

同じ理由で **`meta_fields` は `config` に入れない**。normalizer は `config` を
ハッシュして `ConfigHash` を作るので、混ぜると「実行して保存し直しただけの
ワークフロー」が差分ありになる。`raw_xml` や座標と同じ「diff/normalization
対象外」の扱いにしてある。

判定は `source` 属性(`"SpatialInfo: CentroidObj Source=<入力フィールド>"`)で
行う。出力 MetaInfo は上流から受け継いだ列も載せているため、名前だけでは
「このノードが作った列か」が決まらないからである。ただし `source` に ToolID は
無いので、**同じ入力フィールドを読む Spatial Info が2つあると文字列が完全に
